...
module.exports = function Worker(util, events) {
  ...
  this.weight = function(data){
    ...
    items_collection.find({tags:{$in:["weight"]}, diff:{'$exists':true}}, {sort:{weight_date:-1}, limit:28}).toArray(function(err, docs){
      var weight = _.map(docs.reverse(), function(doc){
        return {
          weight: doc.weight,
  ...
  var process_post = function(post, callback){
    async.waterfall([
      function fetch_original_image(callback){
        console.log('fetch_original_image');
        request({
    ...
    try {
      data.data.tumblog.photo({limit: 10, offset: data.offset}, function(err, response){
        if(err){
          ...
          if(response.statusCode !== '200' || typeof response.posts === 'undefined'){
            console.log('starting the log --------');
            console.log(JSON.stringify(response));
            console.log('error: ending the log <check_tumblr with non 200 response> --------');
            setTimeout(function(){
              that.emit('check_tumblr', {data: data.data, offset: data.offset});
            }, 10000);
          } else {
      ...
      console.log('starting the log --------');
      console.log(err);
      console.log('error: ending try-catch block for <_check_tumblr> --------');
