  ...
  var process_post = function(post, callback){
    async.waterfall([
      ...
      function write_file(response, body, callback){
        ...
        fs.writeFile(post.id + '.jpg', body, 'binary', setTimeout(function(){
          callback;
        }, 2000));
      },
      function extract_exif(callback){
        fs.exists(post.id + '.jpg', function(exists){
          if(exists){
            console.log('extract_exif');
            im.readMetadata(post.id + '.jpg', callback);
          } else {
            console.log('extract_exif -> image not on disk');
            callback(new Error("Image not on disk"), null);
          }
        });
      },
